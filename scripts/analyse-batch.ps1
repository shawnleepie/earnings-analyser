...
if ($otherBatchRunning) {
    Log "ABORTING: another headless claude backfill/analyse process is already running (PID $($otherBatchRunning.ProcessId), command: $($otherBatchRunning.CommandLine)). Wait for it to finish, or confirm it targets different ticker(s) before proceeding manually."
    exit 1
}
...
foreach ($ticker in $Tickers) {
    ...
    if ($freshReport) {
        Log "$ticker - DONE. Report freshly written this run: $($freshReport.Name), at $($freshReport.LastWriteTime). Exit code: $($proc.ExitCode)."
        $results += "$ticker : done ($($freshReport.Name))"
    } else {
        Log "$ticker - WARNING: exited (code $($proc.ExitCode)) but no report was written or updated during this run (checked for files modified after $preRunTime). Check $stdoutLog / $stderrLog manually — an old report may already exist from a prior day, but that does NOT count as success here."
